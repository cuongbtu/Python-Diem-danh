...
def recognizeAttendence():
    ...
    col_names = ['Id','Code','Name','Date','Time']
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                timeStamp = datetime.datetime.fromtimestamp(
                    ts).strftime('%H:%M:%S')
                aa = df.loc[df['Id'] == Id]['Code'].values
                bb = df.loc[df['Id'] == Id]['Name'].values
                # Truy cập hàng và cột trong dataframe df
                tt = str(Id) + "-" + aa + "-" + bb
                attendance.loc[len(attendance)] = [Id, aa, bb, date,
                                                   timeStamp]  
                # Truy cập và lưu vào trong dataframe attendace
    ...
    messagebox.showinfo('Successful', 'Điểm danh thành công')
